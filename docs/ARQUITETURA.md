...
- **E2E — Playwright** (`tests/e2e/*.spec.ts`): fluxos reais sobre o build de produção. O `webServer` faz `BASE_PATH=/ npm run build && preview` para o `base` não partir as rotas. Padrão: helper `completeOnboarding(page)`, **navegação SPA (sem `page.goto` em rotas internas** — o guard de onboarding redireciona após um reload, por isso navega-se sempre por links), e `exact: true`/seletores `href*=` para desambiguar. Ecrãs cobertos: onboarding→desafio, catálogo+pesquisa, horta (adicionar planta), calendário, curso (lição), perfil (export), planeador (canteiro/célula), diagnóstico (sintomas) e glossário (pesquisa).
